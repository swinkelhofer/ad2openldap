...
for elem in entries:
	...
	f.write(elem)
	f.close()
	print os.popen("%s/bin/ldapadd -h %s -p %s -x -w '%s' -D \"cn=ruth,%s\" -f %s/tmp.ldif 2>&1" % (path, host, port, root_pw, ldap_base, path)).read()
...
for elem in entries:
	...
	f.write(elem)
	f.close()
	print os.popen("%s/bin/ldapmodify -h %s -p %s -x -w '%s' -D \"cn=ruth,%s\" -f %s/tmp.ldif 2>&1" % (path, host, port, root_pw, ldap_base, path)).read()

# Find entries to delete
entrystring = os.popen("/usr/bin/perl %s/migrate.pl -u -b '%s' %s/hashes.txt | grep dn: | sed 's/dn: //g'" % (path, ldap_base, path)).read()
entries = entrystring.rstrip().split("\n")

oldentrystring = os.popen("%s/bin/ldapsearch -h %s -p %s -x -w '%s' -b \"%s\" -D \"cn=ruth,%s\" | grep dn: | sed 's/dn: //g'" % (path, host, port, root_pw, ldap_base, ldap_base)).read()
...
for entry in oldentries:
	try:
		...
		print os.popen("%s/bin/ldapdelete -h %s -p %s -x -w '%s' -D \"cn=ruth,%s\" %s 2>&1" % (path, host, port, root_pw, ldap_base, entry)).read()
